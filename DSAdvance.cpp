...
#pragma warning(pop)
	...
	while (!(GetAsyncKeyState(VK_LMENU) & 0x8000 && GetAsyncKeyState(VK_ESCAPE) & 0x8000))
	{
		...
		//@039 Always меняем на button not pressed
			...
			const float InputSize = sqrtf(effGyroX * effGyroX + effGyroY * effGyroY + effGyroZ * effGyroZ) / AppStatus.FrameTime; //3. Расчет физики затухания чувствительности (Snippet by JibbSmart)
